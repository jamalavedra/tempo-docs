...
An RPC node running in follow mode is a **full node**. It:

- Fetches blocks from a trusted RPC endpoint
- Executes every transaction locally through the EVM
- Validates each block after execution
- Stores complete block data and maintains full state

The only trust assumption is block ordering—your node trusts the RPC endpoint to provide the correct sequence of blocks. All execution and validation happens locally on your machine.
